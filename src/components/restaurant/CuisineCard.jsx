...
const CuisineCard = ({ cuisine, variant = 'default', className = '' }) => {
  ...
  if (variant === 'compact') {
    return (
      <Link
        to={`/cuisines/${id}`}
        className={`flex flex-col items-center gap-2 p-4 bg-white rounded-xl border border-gray-100 hover:shadow-md hover:border-primary-200 transition-all group ${className}`}
      >
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-white p-4">
        ...
        <p className="text-sm text-white/80 mt-1">{count} restaurants</p>
      </div>
