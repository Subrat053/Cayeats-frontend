    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Feature your dishes, recipes &amp; images — linked to your
            restaurant profile
          </p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-sm text-gray-600">
            {currentPlan.productsUsed} / {currentPlanData?.products} products
            used
          </span>
        </div>
        <div className="w-full h-3 bg-gray-200 rounded-full overflow-hidden">
          <div
            ...
            style={{
              width: `${(currentPlan.productsUsed / currentPlanData?.products) * 100}%`,
            }}
          />
        </div>
        <div className="flex items-center justify-between mt-2 text-xs text-gray-500">
          <span>
            {currentPlanData?.products - currentPlan.productsUsed} products
            remaining
          </span>
          <span>
            Up to {currentPlanData?.imagesPerProduct} images per product
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          {
            label: "Total Views",
            value: products.reduce((s, p) => s + p.views, 0).toLocaleString(),
            color: "text-blue-600",
          },
          {
            label: "Total Orders",
            value: products.reduce((s, p) => s + p.orders, 0).toLocaleString(),
              ...
              {products.map((product) => (
                <tr key={product.id} className="hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4 text-center">
                    <span
                      className={`text-sm ${product.images >= currentPlanData?.imagesPerProduct ? "text-orange-600" : "text-gray-600"}`}
                    >
                  ...
                  <td className="py-3 px-4 text-center">
                    <Badge
                      variant={
                        product.status === "published" ? "success" : "default"
                      }
                      size="sm"
                    >
        ...
        <Card className="border-2 border-primary">
          ...
          <Card.Content className="pt-6">
            <form
              ...
              onSubmit={(e) => {
                e.preventDefault();
                logger.debug("Adding product:", newProduct);
              }}
            >
          ...
          <Card.Header>
            <div className="flex items-center justify-between">
              <div>
                ...
                <Card.Description>
                  Semi-Annual &amp; Annual only —{" "}
                  <strong>Zero transaction fee</strong> on all plans
                </Card.Description>
              </div>
              <Button
                variant="ghost"
                ...
                >
                  ...
                  <ul className="space-y-2 text-sm text-gray-600 mb-4">
                    <li className="flex items-center gap-2">
                      ...
                      {plan.products} product listings
                    </li>
                    <li className="flex items-center gap-2">
                      <CheckCircle className="w-4 h-4 text-green-500" />
                      {plan.imagesPerProduct} images per product
                    </li>
                    {plan.video && (
                      <li className="flex items-center gap-2">
                        <CheckCircle className="w-4 h-4 text-green-500" />
                  ...
                  {plan.id === currentPlan.tier ? (
                    ...
                    <Button className="w-full">
                      ...
                      {plan.semiAnnual > (currentPlanData?.semiAnnual || 0)
                        ? "Upgrade"
                        : "Switch"}{" "}
                      to {plan.name}
                    </Button>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex gap-3">
        ...
        <div className="text-sm text-blue-700">
          <strong>Product listings</strong> appear on your restaurant profile
          page. There is <strong>zero transaction fee</strong> on all product
          plans. Upgrade your plan to add more products, images, video, store
          maps, and document uploads.
        </div>
      </div>
    </div>
  );
